...
import { IThresholdpresets } from '../../../interface/IThresholdpresets';
...
@Component({
  ...
  templateUrl: './ThresholdPresets.component.html',
  styleUrls: ['./ThresholdPresets.component.scss']
})
export class ThresholdpresetsComponent implements OnInit {
...
{ field: 'Temperature2Min', editable: false, filterParams: this.filterParams as ITextFilterParams },
  ...
  dataSource: IThresholdpresets[];
  selection: SelectionModel<IThresholdpresets>;
  pageNo: number;
  orderBy: string;
  pageSize: number;
  searchValue: string;
  ...
  constructor(private ThresholdPresetsService: ThresholdpresetsService, private router: Router, private toastr: ToastrService) { 
  ...
  onFilterChanged(params: FilterChangedEvent<IThresholdpresets>) {
    ...
    if (filterArray.length > 0) {
      const dataSource: IDatasource = {
        ...
        getRows: (paramRow: IGetRowsParams) => {
          ...
          this.ThresholdPresetsService.filterThresholdpresets(filterArray,this.pageNo + 1, this.pageSize, orAnd, this.orderBy).then((res: any) => {
            if (res.code === 1) {
  ...
  onPaginationChanged(params: PaginationChangedEvent<IThresholdpresets>) {
    this.pageNo = params.api.paginationGetCurrentPage();
    this.ThresholdPresetsService.getThresholdpresets(params.api.paginationGetCurrentPage() + 1, this.pageSize, this.orderBy).then((res: any) => {
      if (res.code === 1) {
  ...
  getData(params) {
    const dataSource: IDatasource = {
      ...
      getRows: (paramRow: IGetRowsParams) => {
        this.pageNo = Math.ceil(paramRow.startRow / this.pageSize);
        this.ThresholdPresetsService.getThresholdpresets(this.pageNo + 1, this.pageSize, "",this.orderBy).then((res: any) => {
          if (res.code === 1) {
            paramRow.successCallback(res.document.records, res.document.totalRecords);
          }
        });
  ...
  deleteItem(row: any) {
    if (confirm("Are you sure, you want to delete?")) {
      this.ThresholdPresetsService.deleteThresholdpresets(row.Id).then((res: any) => {
        if (res.code === 1) {
  ...
  editItem(row: any) {
    this.router.navigateByUrl("ThresholdPresets/" + row.Id);
  }
