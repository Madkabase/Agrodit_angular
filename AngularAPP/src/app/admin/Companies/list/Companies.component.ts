...
import { ICompanies } from '../../../interface/ICompanies';
...
{ field: 'OwnerId', editable: false, filterParams: this.filterParams as ITextFilterParams },
{ field: 'AppId', editable: false, filterParams: this.filterParams as ITextFilterParams },
{ field: 'AppName', editable: false, filterParams: this.filterParams as ITextFilterParams },
{ field: 'MaxDevices', editable: false, filterParams: this.filterParams as ITextFilterParams },
{ field: 'CompanyType', editable: false, filterParams: this.filterParams as ITextFilterParams },

    ];

  dataSource: ICompanies[];
  selection: SelectionModel<ICompanies>;
  pageNo: number;
  orderBy: string;
  pageSize: number;
  searchValue: string;
  ...
  onFilterChanged(params: FilterChangedEvent<ICompanies>) {
    ...
    if (filterArray.length > 0) {
      const dataSource: IDatasource = {
        ...
        getRows: (paramRow: IGetRowsParams) => {
          ...
          this.CompaniesService.filterCompanies(filterArray,this.pageNo + 1, this.pageSize, orAnd, this.orderBy).then((res: any) => {
            if (res.code === 1) {
  ...
  onPaginationChanged(params: PaginationChangedEvent<ICompanies>) {
    this.pageNo = params.api.paginationGetCurrentPage();
    this.CompaniesService.getCompanies(params.api.paginationGetCurrentPage() + 1, this.pageSize, this.orderBy).then((res: any) => {
      if (res.code === 1) {
  ...
  getData(params) {
    const dataSource: IDatasource = {
      ...
      getRows: (paramRow: IGetRowsParams) => {
        this.pageNo = Math.ceil(paramRow.startRow / this.pageSize);
        this.CompaniesService.getCompanies(this.pageNo + 1, this.pageSize, "",this.orderBy).then((res: any) => {
          if (res.code === 1) {
            paramRow.successCallback(res.document.records, res.document.totalRecords);
          }
        });
  ...
  deleteItem(row: any) {
    if (confirm("Are you sure, you want to delete?")) {
      this.CompaniesService.deleteCompanies(row.Id).then((res: any) => {
        if (res.code === 1) {
