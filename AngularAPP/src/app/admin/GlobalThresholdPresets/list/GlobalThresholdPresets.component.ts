...
import { IGlobalthresholdpresets } from '../../../interface/IGlobalthresholdpresets';
...
@Component({
  ...
  templateUrl: './GlobalThresholdPresets.component.html',
  styleUrls: ['./GlobalThresholdPresets.component.scss']
})
export class GlobalthresholdpresetsComponent implements OnInit {
...
{ field: 'Salinity2Min', editable: false, filterParams: this.filterParams as ITextFilterParams },
  ...
  dataSource: IGlobalthresholdpresets[];
  selection: SelectionModel<IGlobalthresholdpresets>;
  pageNo: number;
  orderBy: string;
  pageSize: number;
  searchValue: string;
  ...
  constructor(private GlobalThresholdPresetsService: GlobalthresholdpresetsService, private router: Router, private toastr: ToastrService) { 
  ...
  onFilterChanged(params: FilterChangedEvent<IGlobalthresholdpresets>) {
    ...
    if (filterArray.length > 0) {
      const dataSource: IDatasource = {
        ...
        getRows: (paramRow: IGetRowsParams) => {
          ...
          this.GlobalThresholdPresetsService.filterGlobalthresholdpresets(filterArray,this.pageNo + 1, this.pageSize, orAnd, this.orderBy).then((res: any) => {
            if (res.code === 1) {
  ...
  onPaginationChanged(params: PaginationChangedEvent<IGlobalthresholdpresets>) {
    this.pageNo = params.api.paginationGetCurrentPage();
    this.GlobalThresholdPresetsService.getGlobalthresholdpresets(params.api.paginationGetCurrentPage() + 1, this.pageSize, this.orderBy).then((res: any) => {
      if (res.code === 1) {
  ...
  getData(params) {
    const dataSource: IDatasource = {
      ...
      getRows: (paramRow: IGetRowsParams) => {
        this.pageNo = Math.ceil(paramRow.startRow / this.pageSize);
        this.GlobalThresholdPresetsService.getGlobalthresholdpresets(this.pageNo + 1, this.pageSize, "",this.orderBy).then((res: any) => {
          if (res.code === 1) {
            paramRow.successCallback(res.document.records, res.document.totalRecords);
          }
        });
  ...
  deleteItem(row: any) {
    if (confirm("Are you sure, you want to delete?")) {
      this.GlobalThresholdPresetsService.deleteGlobalthresholdpresets(row.Id).then((res: any) => {
        if (res.code === 1) {
  ...
  editItem(row: any) {
    this.router.navigateByUrl("GlobalThresholdPresets/" + row.Id);
  }
